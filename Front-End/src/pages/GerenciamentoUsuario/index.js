...
export default function GerenciamentoUsuario() {
    ...
    const fecharModal = () => {
        document.getElementById("CaixaDeDialogo").close();
    };

    const mostrarModal = () => {
        document.getElementById("CaixaDeDialogo").showModal();
    };
    ...
    const alterarPermissao = async (nome, cpf, email, senha, privilegio) => {
        try {
            ...
            console.error('Erro ao alterar permissão:', error.response?.data || error.message);
        }
    };
                            ...
                            <table>
                                ...
                                <tbody>
                                    {usuariosFiltrados.map((usuario) => (
                                        <tr key={usuario.cpf} className='Conteudo'>
                                            <td className='primeiro'>{usuario.nome}</td>
                                            <td>{usuario.email}</td>
                                            <td className='final'>
                                                <button onClick={() => alterarPermissao(usuario.nome, usuario.cpf, usuario.email, usuario.senha, usuario.privilegio)}>
                                                    {usuario.privilegio === 'normal' ? (
                                                        <svg width="22" height="25" viewBox="0 0 22 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <g clipPath="url(#clip0_413_130)">
